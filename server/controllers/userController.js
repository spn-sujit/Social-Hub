...
export const unfollowUser = async (req, res) => {
  try {
    ...
    const { id } = req.body;
    const user = await User.findById(userId);
    user.following = user.following.filter((i) => i !== id);
    await user.save();
    const toUser = await User.findById(id);
    toUser.followers = toUser.followers.filter((i) => i !== userId);
    await toUser.save();
    return res.json({
      success: true,
...
export const sendConnectionRequest = async (req, res) => {
  try {
    ...
    } else if (connection.from_user_id.toString() === userId) {
      return res.json({ success: false, message: "Request already sent" });
    } else if (connection.to_user_id.toString() === userId) {
      return res.json({
        success: false,
